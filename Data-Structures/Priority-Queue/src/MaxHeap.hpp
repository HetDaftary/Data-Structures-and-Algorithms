...
template<class T, bool isFixedSize> class MaxHeap {
    public:
        T* arr;
        ...
        MaxHeap(T* arr, int elementCount) {
        ...
        int search(int key) {
            for (int i = 0; i < this -> elementCount; i++) {
                if (key == (this -> arr[i]).priority) return i;
            } return -1;
        }
        ...
        void display(ostream& outStream, string sep = " ") {
            for (int i = 0; i < this -> elementCount; i++) {
                (this -> arr[i]).qu -> displayQueue(outStream, sep);
            } 
        }
};